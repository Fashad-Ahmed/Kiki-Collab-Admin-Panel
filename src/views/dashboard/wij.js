...
const WidgetsDropdown = () => {
  ...
  const totalNumberOfDevelopedGames = games.reduce((acc, curr) => {
    return acc + parseInt(curr.developer)
  }, 0)

  const totalNumberOfActiveGames = games.reduce((acc, curr) => {
    if (curr.active == true) {
      return acc + 1
    }
  }, 0)

  const totalNumberOfRegisteredPhones = games.reduce((acc, curr) => {
    return acc + parseInt(curr.perDay)
  }, 0)
